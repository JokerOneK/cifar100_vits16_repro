...
try:
    ...
    print("Warning: 'mambavision' library not found. Install with: pip install mambavision")

# -------------------------------
# MambaVision Model Configurations
# -------------------------------
MODEL_CONFIGS = {
    "mamba_vision_T": {
        "name": "mamba_vision_T",
        "hf_name": "nvidia/MambaVision-T-1K",
        "lora_targets": ["in_proj", "out_proj", "fc1", "fc2"],
        "params_m": 31.8,
    },
    "mamba_vision_T2": {
        "name": "mamba_vision_T2",
    ...
    "mamba_vision_L": {
        "name": "mamba_vision_L",
        "hf_name": "nvidia/MambaVision-L-1K",
        "lora_targets": ["in_proj", "out_proj", "fc1", "fc2"],
        "params_m": 227.9,
    },
    "mamba_vision_L2": {
        "name": "mamba_vision_L2",
        "hf_name": "nvidia/MambaVision-L2-1K",
        "lora_targets": ["in_proj", "out_proj", "fc1", "fc2"],
        "params_m": 241.5,
    },
}
...
DEFAULT_OUTDIR = Path('./mambavision_results')
...
# -------------------------------
# QLoRA quantize (Linear -> Linear4bit), skip head
# -------------------------------
def quantize_model_in_place(model):
    ...
    def replace_linear(module, name_prefix=""):
        for name, child in module.named_children():
            ...
            if "head" in full_name or "classifier" in full_name:
                continue
...
def build_model(num_classes: int, ckpt: bool, peft_method: str, args):
    ...
    try:
        ...
        if MAMBAVISION_AVAILABLE:
            print("[Loading] Trying mambavision pip package...")
            ...
            raise ImportError(
                f"Cannot load MambaVision model.\n"
                f"Option 1: pip install transformers (recommended)\n"
                f"Option 2: pip install mambavision (requires CUDA + nvcc)\n"
                f"Error: {e}"
            )
    ...
    if peft_method == "bitfit":
        ...
        for name, param in model.named_parameters():
            ...
            if "bias" in name or "head" in name or "classifier" in name:
                param.requires_grad = True

    elif peft_method == "qlora":
        if not PEFT_AVAILABLE:
            raise ImportError("peft required for QLoRA")
        model = quantize_model_in_place(model)
    ...
    if peft_method in ["lora", "qlora", "adalora"]:
        ...
        # AdaLora не поддерживает Conv2d
        if peft_method == "adalora":
            has_conv_proj = any(
                isinstance(m, nn.Conv2d) and 'proj' in n
                for n, m in model.named_modules()
            )
            if has_conv_proj and "proj" in target_modules:
...
def attach_mem_hooks(model: nn.Module, memlog: MemLogger, epoch_ref, step_ref):
    ...
    blocks = get_all_blocks_for_hooks(model)
        ...
        def make_forward(b, orig_fwd, layer_idx):
            def forward(*args, **kwargs):
                ...
                def run_block(inp):
                    ...
                    try:
                        # Вызываем orig_fwd с inp как первым аргументом
                        if args:
                            out = orig_fwd(inp, *args[1:], **kwargs)
                        else:
                            out = orig_fwd(inp, **{k: v for k, v in kwargs.items() if k not in ('hidden_states', 'x')})
                    finally:
...
def train(args):
    ...
    if args.ckpt_mode == "adaptive":
        inject_dynamic_checkpointing(
            ...
            pwr=pwr, threshold_ratio=args.threshold_ratio
        )
