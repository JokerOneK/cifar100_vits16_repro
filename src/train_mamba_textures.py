...
class VimBlock(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, rim=True):
        super().__init__()
...
def load_pretrained_weights(model, model_name="vim_small_midclstok"):
    ...
    if model.head.weight.shape[0] != new_state_dict.get("head.weight", torch.empty(0)).shape[0]:
        print(f"Head mismatch (Pretrained: {new_state_dict.get('head.weight', torch.tensor([])).shape}, Current: {model.head.weight.shape}). Dropping head weights.")
...
def train_one_run(dataset_name: str, batch_size: int, peft_method: str, args, device: torch.device, base_outdir: Path):
    ...
    if args.pretrained:
        ...
        print("Using Mid-Cls-Token Strategy for Pretrained Weights")
    ...
    if peft_method == "qlora" and BNB_AVAILABLE:
        print("Applying QLoRA (4-bit placeholder/quantization not fully implemented here, using standard LoRA + load checks if needed)...")
    ...
    if peft_method in ["lora", "qlora"]:
        ...
        print(f"BitFit: Trainable params: {trainable_count/1e6:.2f}M / {total_count/1e6:.2f}M ({trainable_count/total_count*100:.2f}%)")
...
def main():
    ...
    parser.add_argument("--datasets", type=str, nargs='+', default=["dtd", "cifar100"], help="List of datasets")
    parser.add_argument("--batch_sizes", type=int, nargs='+', default=[32, 16], help="List of batch sizes")
    ...
    parser.add_argument("--peft", type=str, choices=["none", "lora", "qlora", "adalora", "bitfit", "all"], default="none")
