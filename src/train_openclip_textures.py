"""
OpenCLIP ViT-L/14 Zero-Shot Evaluation on CIFAR-100 / DTD
Adapted from openclip-vit-l-14.ipynb for local GPU (RTX 2060 Super 8GB).
...
Install:
  pip install open_clip_torch pynvml pandas
"""
...
# ========================= CONFIG =========================
MODEL_SIZE      = "large"                # "base" | "large" | "huge"
...
OUTDIR     = SCRIPT_DIR / "openclip_results"
DATA_ROOT  = SCRIPT_DIR / "data"
GPU_INDEX  = 0

OPENCLIP_MODELS = {
    "base":  {"arch": "ViT-B-16",  "pretrained": "datacomp_l_s1b_b8k",    "params_m": 86},
    "large": {"arch": "ViT-L-14",  "pretrained": "datacomp_xl_s13b_b90k", "params_m": 304},
    "huge":  {"arch": "ViT-H-14",  "pretrained": "laion2b_s32b_b79k",     "params_m": 632},
}
...
class CLIPDataset(Dataset):
    """Returns (preprocessed tensor, label_idx, class_name)."""
    def __init__(self, base_dataset, class_names, preprocess):
    ...
    def __getitem__(self, idx):
        ...
        return self.preprocess(img), label, self.class_names[label]


def make_test_loader(dataset_name, preprocess):
    ...
    wrapped = CLIPDataset(ds, class_names, preprocess)
...
# ========================= MODEL =========================
@torch.no_grad()
def build_class_embeddings(model, tokenizer, class_names, templates, text_bs=64):
...
def build_openclip(model_size):
    info = OPENCLIP_MODELS[model_size]
    print(f"[OpenCLIP] Loading {info['arch']} / {info['pretrained']} (~{info['params_m']}M params)...")
    model, _, preprocess = open_clip.create_model_and_transforms(
        info["arch"], pretrained=info["pretrained"])
    model = model.to(device).half()
    model.eval()
    tokenizer = open_clip.get_tokenizer(info["arch"])
    print(f"[OpenCLIP] Total params: {sum(p.numel() for p in model.parameters())/1e6:.1f}M")
...
@torch.no_grad()
def run_eval_discriminative(model, test_loader, text_embs, pwr, epoch_idx=-1):
    model.eval()
    correct = total = 0
    start_t = time.time()
    ...
    for step, (images, labels, _) in pbar:
        ...
        img_f = model.encode_image(images).float()
        ...
        pbar.set_postfix(acc=f"{acc_so_far:.2f}%")

    final_acc = 100.0 * correct / total if total > 0 else 0.0
    elapsed = time.time() - start_t
    print(f"  Accuracy: {final_acc:.2f}%  ({correct}/{total})  Time: {elapsed:.1f}s")
    return final_acc, elapsed


# ========================= MAIN =========================
def run_experiment(dataset_name, model, tokenizer, preprocess):
    run_tag = f"{dataset_name}/openclip_{MODEL_SIZE}"
    print(f"\n{'='*80}\nSTARTING: {run_tag}\n{'='*80}")
    set_seed(SEED)

    ab_vals = [1, 2, 3, 4, 5]
    run_dir = OUTDIR / dataset_name / f"openclip_{MODEL_SIZE}"
    ...
    test_loader, class_names, templates = make_test_loader(dataset_name, preprocess)
    ...
    try:
        ...
        text_embs = build_class_embeddings(model, tokenizer, class_names, templates)
        print(f"    text_embs: {text_embs.shape}")

        print(">>> Zero-shot evaluation...")
        pwr.reset_epoch()
        acc, eval_time = run_eval_discriminative(
            model, test_loader, text_embs, pwr, epoch_idx=0)
...
if __name__ == "__main__":
    ...
    model, tokenizer, preprocess = build_openclip(MODEL_SIZE)

    results = {}
    for ds in DATASETS:
        try:
            csv_path = run_experiment(ds, model, tokenizer, preprocess)
            results[ds] = csv_path
    ...
    for ds, csv_path in results.items():
        print(f"\n{'='*60}")
        print(f"  {ds.upper()} - OpenCLIP-{MODEL_SIZE.upper()}")
        print(f"{'='*60}")
