...
MODEL_CONFIGS = {
    ...
    "tiny_vit": {
        "name": "tiny_vit_21m_224.in1k",
        "lora_targets": ["qkv", "proj", "fc1", "fc2"],
        "params_m": 21.2,
    },
    "levit": {
        "name": "levit_256.fb_dist_in1k",
        "lora_targets": ["qkv.linear", "proj.ln.linear", "ln1.linear", "ln2.linear"],
        "params_m": 18.9,
    },
}
...
SEED = 42
MEMORY_CAPACITY_GB = 2
DEFAULT_OUTDIR = Path('./tinyvit_texture_results')
...
# -------------------------------
# Universal block extraction
# -------------------------------
def get_model_blocks(model) -> list:
    """
    Универсально получает ВСЕ блоки из модели (architecture-agnostic).
    Включает все типы: MBConv, TinyVitBlock, LevitBlock, MambaBlock и др.
    Это позволяет adaptive checkpointing покрывать всю сеть.
    Поддерживает: ViT, DeiT, TinyViT, LeViT, SSM модели.
    """
    base = get_base_model_from_peft(model)

    # ViT, DeiT - прямой доступ к blocks
    if hasattr(base, 'blocks') and not hasattr(base, 'stages'):
        return list(base.blocks)

    # TinyViT, LeViT, гибридные модели - собираем ВСЕ блоки из stages
    ...
    raise ValueError(f"Unknown model architecture: {type(base).__name__}")
...
def build_model(num_classes: int, ckpt: bool, peft_method: str, args):
    ...
    if peft_method in ["lora", "qlora", "adalora"]:
        ...
        # Используем LoRA targets из конфигурации модели или из аргументов
        if args.lora_targets:
            target_modules = [t.strip() for t in args.lora_targets.split(",") if t.strip()]
        else:
            target_modules = model_cfg["lora_targets"].copy()

        # Важно: AdaLora в peft не поддерживает Conv2d
        # Для ViT/TinyViT убираем 'proj' если он матчится на patch_embed.proj
        if peft_method == "adalora" and "proj" in target_modules:
            # Проверяем есть ли Conv2d с именем proj
            has_conv_proj = any(
                isinstance(m, nn.Conv2d) and 'proj' in n
                for n, m in model.named_modules()
            )
            if has_conv_proj:
                target_modules = [t for t in target_modules if t != "proj"]
                print(f"[AdaLora] target_modules adjusted (removed 'proj'): {target_modules}")

        print(f"[{peft_method.upper()}] LoRA target modules: {target_modules}")
    ...
    n_blocks = len(blocks)
    assert n_blocks > 0, f"No transformer blocks found in {model_name}"
...
def attach_mem_hooks(model: nn.Module, memlog: MemLogger, epoch_ref, step_ref):
    """Attach memory/timing hooks to transformer blocks"""
    handles = []
    ...
    # Получаем блоки универсальным способом
    blocks = get_model_blocks(model)
...
def inject_dynamic_checkpointing(model: nn.Module, device: torch.device, mem_cap_bytes: int, step_ref, memlog: MemLogger, epoch_ref, pwr: GpuPowerMeter = None, threshold_ratio: float = 0.8):
    """Inject adaptive gradient checkpointing into transformer blocks"""
    blocks = get_model_blocks(model)
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Train ViT/TinyViT/LeViT on DTD/CIFAR-100 with PEFT methods and adaptive checkpointing"
    )

    # Model selection
    ap.add_argument("--model", type=str, default=DEFAULT_MODEL,
                    choices=list(MODEL_CONFIGS.keys()),
                    help=f"Model architecture: {list(MODEL_CONFIGS.keys())}")
    ...
    # Multi-experiment: datasets, peft methods, checkpoint modes
    ap.add_argument("--datasets", nargs="+", default=["cifar100", "dtd"],
                    choices=["cifar100", "dtd"],
                    help="Datasets to run experiments on")
    ap.add_argument("--peft-methods", nargs="+", default=["none", "bitfit", "lora", "adalora", "qlora"],
                    choices=["none", "bitfit", "lora", "qlora", "adalora"],
                    help="PEFT methods to test")
    ap.add_argument("--ckpt-modes", nargs="+", default=["none", "static", "adaptive"],
                    choices=["none", "static", "adaptive"],
                    help="Checkpointing modes to test")
    ...
    ap.add_argument("--download", action="store_true", help="Download dataset")
    ...
    ap.add_argument("--lora-targets", type=str, default="",
                    help="Override LoRA target modules (comma-separated). If empty, uses model defaults.")
...
def train_single_run(args, dataset: str, peft_method: str, ckpt_mode: str):
    ...
    if peft_method == "none":
        lr = args.lr_fullft
        print(f"[LR] {lr:.1e} (FullFT)")
    else:
        lr = args.lr
        print(f"[LR] {lr:.1e} ({peft_method})")
    ...
    n_blocks = len(get_model_blocks(base_model_for_hooks))
...
def main():
    ...
    peft_methods = args.peft_methods
    ckpt_modes = args.ckpt_modes

    total = len(datasets_list) * len(peft_methods) * len(ckpt_modes)
    print(f"\n{'#' * 80}")
    print(f"EXPERIMENT MATRIX: {total} runs")
    print(f"  Datasets:    {datasets_list}")
    print(f"  PEFT:        {peft_methods}")
    print(f"  Checkpoints: {ckpt_modes}")
